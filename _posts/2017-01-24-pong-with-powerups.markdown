---
published: true
layout: post
title: Pong with Power Ups
categories: 'javascript js crafty game'
img: pong-with-powerups.png
---
<a target="_blank"  href="https://pateldhruv1993.github.io/pong_game/" class="btn btn-primary">Play!</a>
<a  target="_blank" href="https://github.com/pateldhruv1993/pong_game" class="btn btn-primary">GitHub Link</a>


<h1>What is this?</h1>
A simple game of Pong made using Craftyjs and Javascript. The game has an AI and you, the player. When either one of you hit the ball there is a small chance a power up will spawn for the enemy. If you get the power up you can use it by pressing SPACEBAR. Power Ups die out within 3 seconds of them spawning in.

<br><br>
<a target="_blank" href="https://pateldhruv1993.github.io/pong_game/" class="btn btn-primary">Play!</a>

<h1>Screenshots:</h1>
<img width="100%" src="{{site.baseurl}}/images/pong-with-powerups1.png">
<br><br>
<img width="100%" src="{{site.baseurl}}/images/pong-with-powerups2.png">
